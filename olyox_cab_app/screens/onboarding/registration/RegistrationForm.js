...
export default function ImprovedRegistrationForm() {
  ...
  const handleSubmit = async () => {
    if (validate(2)) {
      setLoading(true)
      try {
        const response = await axios.post("http://192.168.1.2:3000/api/v1/parcel/register_parcel_partner", formData)
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      const response = await axios.post("http://192.168.1.2:3000/api/v1/parcel/login_parcel_otp_verify", {
  ...
  const handleResendOtp = async () => {
    try {
      ...
      await axios.post("http://192.168.1.2:3000/api/v1/parcel/login_parcel_otp_resend", { number: formData.phone })

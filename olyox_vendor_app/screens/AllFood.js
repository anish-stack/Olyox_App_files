...
const AllFood = () => {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              await axios.delete(`http://192.168.1.8:3100/api/v1/tiffin/delete_tiffin_listing/${id}`);
  ...
  const handleUpdateFoodAvailableStatus = async (id, food_availability) => {
    try {
      ...
      const res = await axios.put(`http://192.168.1.8:3100/api/v1/tiffin/update_available_food_status/${id}`, { food_availability: updatedStatus });
      if (res.data.success) {

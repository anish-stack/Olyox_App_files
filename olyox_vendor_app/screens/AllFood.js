...
const AllFood = () => {
  ...
  const handleFetchFood = async () => {
    try {
      ...
      const { data } = await axios.get(
        'http://192.168.1.4:3000/api/v1/tiffin/get_food_by_resutrant_id', {
        headers: {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              await axios.delete(`http://192.168.1.4:3000/api/v1/tiffin/delete_tiffin_listing/${id}`);
  ...
  const handleUpdateFoodAvailableStatus = async (id, food_availability) => {
    try {
      ...
      const res = await axios.put(`http://192.168.1.4:3000/api/v1/tiffin/update_available_food_status/${id}`, { food_availability: updatedStatus });
      if (res.data.success) {

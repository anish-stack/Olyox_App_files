...
export function NewOrder() {
        ...
        const fetchProfile = async () => {
            try {
                ...
                const { data } = await axios.get(
                    'http://192.168.1.4:3000/api/v1/tiffin/get_single_tiffin_profile',
                    {
                        headers: {
    ...
    const handleFetchOrderDetails = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get(`http://192.168.1.4:3000/api/v1/tiffin/get_order_for_resturant/${restaurantId}`);
            if (data.success) {
    ...
    const handleChangeOrderStatus = async (orderId, status) => {
        try {
            ...
            const { data } = await axios.put(`http://192.168.1.4:3000/api/v1/tiffin/update_order_status/${orderId}`, { status: status });
            if (data.success) {

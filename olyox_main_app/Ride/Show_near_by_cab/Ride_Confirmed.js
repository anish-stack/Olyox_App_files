...
export function RideConfirmed() {
    ...
    const fetchReason = async () => {
        try {
            const { data } = await axios.get(`http://192.168.1.12:3100/api/v1/admin/cancel-reasons?active=active`)
    ...
    const fetchRideDetailsFromDb = async () => {
        try {
            ...
            const { data } = await axios.get(`http://192.168.1.12:3100/api/v1/rides/find-ride_details?id=${rideId}`);
            if (data.data) {

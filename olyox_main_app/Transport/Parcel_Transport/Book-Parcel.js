...
const ParcelBooking = () => {
  ...
  const findDistanceAndEtaOFPriceAndTime = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post("http://192.168.11.28:3000/geo-code-distance", {
        pickup,
        dropOff: dropoff,
      });
      setEtaData(data);
  ...
  const handleBookNow = async () => {
    ...
    try {
      const { data } = await axios.post("http://192.168.11.28:3000/api/v1/parcel/request_of_parcel", payload, {
        headers: {

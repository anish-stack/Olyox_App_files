...
const Checkout = () => {
  ...
  const findCurrent = useCallback(async () => {
    try {
      const data = await axios.post(`https://www.appapi.olyox.com/Fetch-Current-Location`, {
  ...
  const fetchCoupons = useCallback(async () => {
    ...
    try {
      const response = await axios.get("https://www.appapi.olyox.com/api/v1/tiffin/tiffin-coupons")
  ...
  const handlePlaceOrder = useCallback(async () => {
    ...
    try {
      const response = await axios.post(
        `https://www.appapi.olyox.com/api/v1/tiffin/create_order_of_food`,
        {
          order_items: items,

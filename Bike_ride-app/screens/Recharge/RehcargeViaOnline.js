...
export default function RechargeViaOnline() {
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      const response = await axios.get('http://192.168.1.23:3100/api/v1/rider/user-details', {
  ...
  const initiatePayment = async (memberId) => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.1.23:3100/api/v1/rider/recharge-wallet/${memberId}/${userData?.BH}`
      );
      ...
      const verifyResponse = await axios.post(
        `http://192.168.1.23:3100/api/v1/rider/recharge-verify/${userData?.BH}`,
        {

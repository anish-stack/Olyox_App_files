...
export default function DeliveryTracking() {
  ...
  const handleFetchDetails = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`http://192.168.1.11:3100/api/v1/parcel/get-parcel/${parcelId}`);
  ...
  const handleChangeStatus = useCallback(async (status) => {
    try {
      ...
      await axios.post('http://192.168.1.11:3100/api/v1/parcel/parcel-status-update', {
        parcelId,
        status,
      });

...
const ParcelHome = () => {
  ...
  const fetchUserDetails = async () => {
    ...
    try {
      ...
      if (response.data.success && response.data.partner) {
        ...
        const dashboard = await axios.get(`http://192.168.1.11:3100/api/v1/rides/parcelDashboardData/${partnerData?._id}`);
  ...
  const updateDriverLocation = useCallback(async (latitude, longitude) => {
    try {
      await axios.post('http://192.168.1.11:3100/webhook/cab-receive-location', {
